...
public class BasedGestureHandle : MonoBehaviour {
    ...
    //public ParticleSystem trackLeft;
    ...
    protected void UpdateUIandHandleControl() {
        ...
        Debug.Log("MAU MASUK TRACKING");
        //float triggerKeyValue = OVRInput.Get(OVRInput.RawAxis1D.RIndexTrigger);
        
        //if (triggerKeyValue > 0.8f) {
        //    if (track.isPlaying) track.Stop();
         //   Debug.LogWarning(triggerKeyValue);
       //     track.Play();
       // } else if (triggerKeyValue < 0.1f) {
        //    track.Stop();
       // }

        Boolean triggerKeyValue = OVRInput.Get(OVRInput.Button.One);
        if (triggerKeyValue)
        {
            if (track.isPlaying) track.Stop();
            track.Play();
        }
        else if (!triggerKeyValue)
        {
            track.Stop();
        }

        float triggerKeyValueLeft = OVRInput.Get(OVRInput.RawAxis1D.LIndexTrigger);
        if (triggerKeyValueLeft > 0.8f)
        {
            //if (trackLeft.isPlaying) trackLeft.Stop();
            //trackLeft.Play();
        }
        else if (triggerKeyValueLeft < 0.1f)
        {
            //trackLeft.Stop();
        }
